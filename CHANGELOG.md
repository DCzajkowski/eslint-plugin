...
### Fixed
- **BREAKING**: commonjs export is now using default export.
```diff
-const { default: dczajkowski } = require('@dczajkowski/eslint-plugin')
+const dczajkowski = require('@dczajkowski/eslint-plugin')
````

## [1.1.0] - 2026-01-09

### Added
- `ordered-imports` rule now sorts import specifiers. The rule should behave closer to how VSCode sorts imports when using the "Sort Imports" feature.

## [1.0.1] - 2026-01-08

### Changed
- Improved performance when using `ordered-imports` rule
